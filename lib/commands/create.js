  ...
  for (const step of steps) {
    ...
    if (action === 'call') {
      ...
      if (fs.existsSync(helperPath) && step.op) {
        ...
        if (op) {
          const args = (step.args || []).map(a => typeof a === 'string' ? substituteVars(a) : a);
    ...
    if (action === 'create-if') {
      if (!featureFlags[step.feature]) {
        if (verbose) console.log(`    ${dim('-')} ${step.path} ${dim(`[create-if: ${step.feature} disabled]`)}`);
        continue;
      }
      ...
      if (tmpl) {
        ...
        console.log(`    ${green('+')} ${step.path} ${verbose ? dim(`[create-if: ${step.feature}]`) : ''}`);
      }
      continue;
    }
