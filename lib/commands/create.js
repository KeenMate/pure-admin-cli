...
/**
 * Convert a CLI flag (e.g. "--profile-panel" or "--no-ecto") to its camelCase opts key.
 * "--profile-panel" → "profilePanel"
 * "--no-ecto" → "noEcto"
 */
function flagToOpt(flag) {
  return String(flag).replace(/^--/, '').replace(/-([a-z])/g, (_, c) => c.toUpperCase());
}
...
async function cmdCreate(appName, opts, usage) {
  ...
  // Module name: kebab/snake_case → PascalCase. e.g. "my-app" or "my_app" → "MyApp"
  const appModule = appName.split(/[-_]/).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('');
  // snake_case form of the app id (for Elixir/Phoenix and other languages that
  // don't allow hyphens in module/package identifiers). e.g. "my-app" → "my_app"
  const appIdSnake = appName.replace(/-/g, '_');
  // defaultMode: declared early so substituteVars can read it during scaffold step.
  // May be refined later from theme manifest (see "Resolve default mode" section).
  ...
  function substituteVars(text) {
    ...
    sub('APP_ID', appName);
    sub('APP_ID_SNAKE', appIdSnake);
    sub('APP_MODULE', appModule);
    sub('CREATE_COMMAND', createCommand);
    ...
    // SCAFFOLD_FLAGS is set after recipe is fetched and features are resolved
    sub('SCAFFOLD_FLAGS', substituteVars._scaffoldFlags || '');
    ...
    if (substituteVars._pageModule) { sub('PAGE_MODULE', substituteVars._pageModule); }
  ...
  function setupFeatureState() {
    ...
    substituteVars._scaffoldFlags = scaffoldFlagsStr;
  ...
  const pageSteps = pages.map(p => {
    ...
    // Module name from entity: snake_case → PascalCase. e.g. "users" → "Users", "user_profile" → "UserProfile"
    const pageModule = entity.split(/[-_]/).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('');
    return {
      action: 'create',
      path: route,
  ...
  for (const step of steps) {
    ...
    if (action === 'create-if') {
      ...
      if (tmpl) {
        ...
        substituteVars._pageModule = step._pageModule || null;
