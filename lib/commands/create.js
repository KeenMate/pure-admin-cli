...
const { copyDirSync, extractZip, mkdir, writeFile, removeFile, removeDir, copyFile } = require('../helpers/files');
...
async function cmdCreate(appName, opts, usage) {
  ...
  // Read themesDir once: app's pureadmin.json > recipe > default
  ...
  // Track what we resolved so we can write the lockfile after the loop.
  const lockEntries = {};
  ...
  for (const id of themeIds) {
    ...
    try {
      mkdir(path.join(appDir, themesDir));
      await downloadFile(`/api/themes/${id}/download`, zipPath);
      extractZip(zipPath, themeDir);
      removeFile(zipPath);
      // Resolved metadata for lockfile; sourced from the API response we
      // already fetched in step 3 (themesData).
      const themeMeta = themesData?.find(t => t.slug === id) || {};
      lockEntries[id] = {
        version: themeMeta.latest || null,
        content_sha: themeMeta.content_sha || null,
  ...
  // Write pureadmin.lock.json so the new project is install-ready (e.g. its
  // CI can immediately run `pureadmin themes install`).
  if (Object.keys(lockEntries).length > 0) {
    const lockPath = path.join(appDir, 'pureadmin.lock.json');
    const lockData = { _format: 1, themes: {} };
    for (const slug of Object.keys(lockEntries).sort()) lockData.themes[slug] = lockEntries[slug];
