...
async function cmdCreate(appName, opts, usage) {
  ...
  if (effectiveTemplatePath) {
    ...
    if (fs.existsSync(helperPath)) {
      try {
        ...
        console.error(`\n  ${bold('Error:')} failed to load template.helper.js: ${e.message}`);
        process.exit(1);
      }
    }
  }
  ...
  const pageSteps = pages.map(p => {
    ...
    if (!pt) {
      console.error(`\n  ${bold('Error:')} page type "${p.type}" is not declared in template.json pageTypes.`);
      process.exit(1);
    }
    if (!pt.file) {
      console.error(`\n  ${bold('Error:')} pageType "${p.type}" must declare a "file" field (template source path).`);
      process.exit(1);
    }
    if (!pt.route) {
      console.error(`\n  ${bold('Error:')} pageType "${p.type}" must declare a "route" field (destination path pattern).`);
      process.exit(1);
    }
