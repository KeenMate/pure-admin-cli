      ...
      if (usedIcons.size > 0) {
        ...
        function injectImports(dir) {
          for (const entry of fs.readdirSync(dir)) {
            ...
            if (!entry.endsWith('.svelte')) continue;
            let content = fs.readFileSync(p, 'utf-8');
            // Check which icons this file uses
            ...
            // Insert after the last import line in <script>
            ...
            if (scriptMatch) {
              ...
              if (lastImportIdx >= 0) {
                ...
                const insertAt = scriptMatch.index + '<script'.length + scriptMatch[0].indexOf('>') - scriptMatch.index + 1 + lineEnd + 1;
                content = content.slice(0, insertAt) + fileImports + '\n' + content.slice(insertAt);
