...
async function cmdCreate(appName, opts, usage) {
  ...
  if (effectiveTemplatePath) {
    ...
    if (iconProvider === 'lucide' && usedLucideIcons.size > 0) {
      function injectLucideImports(dir) {
        for (const entry of fs.readdirSync(dir)) {
          if (['node_modules', '.svelte-kit', '.git'].includes(entry)) continue;
          const p = path.join(dir, entry);
          if (fs.statSync(p).isDirectory()) { injectLucideImports(p); continue; }
          if (!entry.endsWith('.svelte')) continue;
          let content = fs.readFileSync(p, 'utf-8');

          // Find which tracked Lucide icons this file actually uses
          const fileIcons = [...usedLucideIcons.entries()]
            .filter(([component]) => content.includes(`<${component} `));
          if (fileIcons.length === 0) continue;

          const fileImports = fileIcons
      ...
      injectLucideImports(path.join(appDir, 'src'));
    }
  }
