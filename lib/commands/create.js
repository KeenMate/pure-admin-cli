...
const { resolveIconMarkup, resolveIconProvider } = require('../create/icons');
...
async function cmdCreate(appName, opts, usage) {
  ...
  function buildCreateCommand() {
    ...
    if (opts.noIcons) parts.push('--no-icons');
    ...
    const handledOpts = new Set([
      'template', 'templatePath', 'themes', 'theme', 'fontAwesome', 'lucide', 'heroicons', 'fluentUi', 'noIcons',
  ...
  // Icon provider resolution — phase 1 (pre-recipe).
  //
  // Picks a provider from CLI flags / preset / company. If none of those set one,
  // leaves provider=null so phase 2 (post-recipe, before helper.prepare) can
  // honor the template manifest's declared default (features.icons.cli[].isDefault).
  //
  // Phase 1 also surfaces conflicts early — e.g. --no-icons combined with
  // --font-awesome — so the user sees the error before scaffolding starts.
  let iconProvider, iconProviderSource;
  try {
    const initial = resolveIconProvider({ opts, preset, company, companyId, recipe: null });
    // Treat the "no manifest available yet" fallback as pending — phase 2
    // will revisit and honor recipe defaults.
    if (initial.source === 'default (no icon flag)') {
      iconProvider = null;
      iconProviderSource = null;
    } else {
      iconProvider = initial.provider;
      iconProviderSource = initial.source;
    }
  } catch (e) {
  ...
  function resolveIcon(name) {
    return resolveIconMarkup(name, iconProvider, usedLucideIcons, ctx.recipe?.technology);
  }
  ...
  // Expose the finalized recipe to preparators (setProfiles / collectCreateSummary)
  // so they can introspect features and steps (e.g. for demo-pages shadow detection).
  ctx.recipe = recipe;
  ...
  // Icon provider — phase 2 (post-recipe finalization).
  //
  // If phase 1 left the provider pending (no flag/preset/company), honor the
  // template's declared default via recipe.features.icons.cli[].isDefault.
  // Falls back to 'none' if the manifest doesn't declare one.
  if (iconProvider === null) {
    const finalIcon = resolveIconProvider({ opts, preset, company, companyId, recipe });
    iconProvider = finalIcon.provider;
    iconProviderSource = finalIcon.source;
    ctx.iconProvider = iconProvider;
    provenance.iconProvider = iconProviderSource;
    if (verbose) console.log(dim(`    iconProvider resolved: ${iconProvider} (${iconProviderSource})`));
  }
  ...
  const knownOpts = new Set([
    ...
    'heroicons', 'fluentUi', 'noIcons', 'profilePanel', 'settingsPanel', 'local', 'offline',
  ...
  const pageSteps = pages.map(p => {
    ...
    // Route supports {{entity}} (raw) and {{module}} (PascalCase) substitutions
    const route = pt.route
      .replace(/\{\{entity\}\}/g, entity)
      .replace(/\{\{module\}\}/g, pageModule);
    return {
      action: 'create',
      path: route,
      template: pt.file,
  ...
  // Icons are a special case: derive from the finalized iconProvider so
  // template-default selection (e.g. FA default for elixir-phoenix-liveview)
  // is honored even when no CLI flag was passed.
  //
  // Synthetic per-provider flags (`font-awesome`, `heroicons`, `lucide`)
  // exist so recipe steps can gate provider-specific scaffold — e.g.
  // `{ if: "lucide" }` to ship an icon_callback module + SVG assets.
  const featureFlags = { ...(resolvedFeatures || {}) };
  featureFlags['icons'] = iconProvider !== 'none';
  featureFlags['font-awesome'] = iconProvider === 'font-awesome';
  featureFlags['heroicons'] = iconProvider === 'heroicons';
  featureFlags['lucide'] = iconProvider === 'lucide';
  ...
  for (const step of steps) {
    ...
    // Use the already-loaded helper (from effectiveTemplatePath / bundled
    // fallback) — re-requiring from appDir fails when the app's package.json
    // has "type": "module" (Node treats the .js file as ESM and rejects the
    // helper's top-level require() calls).
    if (action === 'call') {
      if (templateHelper && step.op) {
        const op = templateHelper.operations?.[step.op];
        if (op) {
  ...
  // 6b. Strip leftover icon mentions when iconProvider is 'none'.
  // After substitution, per-provider placeholders resolve to "" — leaving
  // visible `icon=""` attrs and (for Phoenix) inline space where __ICON:name__
  // used to be. Sweep those out so the generated source has no mention of
  // icons at all, matching --no-icons intent.
  if (iconProvider === 'none') {
    const scanExts = ['.heex', '.ex', '.exs', '.svelte', '.html', '.ts', '.js'];
    const skipDirs = new Set(['node_modules', '.svelte-kit', '.git', 'build', 'dist', '_build', 'deps']);
    let stripped = 0;
    function stripIcons(dir) {
      for (const entry of fs.readdirSync(dir)) {
        if (skipDirs.has(entry)) continue;
        const p = path.join(dir, entry);
        const stat = fs.statSync(p);
        if (stat.isDirectory()) { stripIcons(p); continue; }
        if (!scanExts.some(e => entry.endsWith(e))) continue;
        const original = fs.readFileSync(p, 'utf-8');
        let content = original;
        // Empty icon="" / icon='' attrs (with the preceding whitespace).
        content = content.replace(/\s+icon=(""|'')/g, '');
        // Lines that became blank after ICON_CDN substitution (only consecutive
        // blank-line runs are collapsed — single blank lines stay).
        content = content.replace(/\n[ \t]+\n[ \t]+\n/g, '\n\n');
        if (content !== original) {
          writeFile(p, content);
          stripped++;
          if (verbose) console.log(`    ${dim('~')} ${path.relative(appDir, p)} ${dim('[stripped empty icon attrs]')}`);
        }
      }
    }
    stripIcons(appDir);
    if (stripped > 0) console.log(`  Stripped icon references from ${stripped} file(s) ${dim('(--no-icons)')}`);
  }
