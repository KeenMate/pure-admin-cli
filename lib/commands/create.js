...
function resolveFeatures(features, opts, preset) {
  ...
  for (const [id, feat] of Object.entries(features)) {
    ...
    if (Array.isArray(feat.cli)) {
      // Multi-option group: enabled if any option flag is set, or any option has isDefault
      let isOn = false;
    ...
    if (typeof feat.cli === 'string') {
      const flag = feat.cli;
      const optKey = flagToOpt(flag);
      // Negative form: --no-X. Truthy opts.noX means feature is DISABLED.
      // Also accept positive opts.X to re-enable (overrides --no-X).
      if (flag.startsWith('--no-')) {
        ...
        if (opts[baseKey] !== undefined) {
          ...
          enabled[id] = feat.isDefault !== false; // default: enabled
        }
      } else {
    ...
    // No CLI flag — purely default-driven
    enabled[id] = feat.isDefault === true;
  }

  // Apply preset overrides (preset.features map: { featureId: bool })
  ...
  // Auto-enable required dependencies (transitive)
  let changed = true;
  while (changed) {
    changed = false;
    for (const [id, feat] of Object.entries(features)) {
 ...
 * For each feature, collects:
 *   - feat.scaffoldFlag (or feat.scaffoldFlagWhenDisabled) when feature is DISABLED
 *   - feat.scaffoldFlagWhenEnabled when feature is ENABLED
 * Joins with spaces.
 */
...
async function cmdCreate(appName, opts, usage) {
  ...
  const ctx = {
    ...
    // Feature toggles (kept for b/c with closure code that still reads them)
    includeFontAwesome,
    includeProfilePanel,
   ...
   *
   * Pipeline:
   *   1. Every entry in ctx.placeholders is substituted (both {{KEY}} and __KEY__ forms)
   *   2. `__ICON:name__` is resolved per icon provider (not a plain placeholder)
   *   3. Legacy Mustache-style conditional blocks are processed
   *
  ...
  function substituteVars(text) {
    ...
    for (const [key, value] of Object.entries(ctx.placeholders)) {
      ...
      result = result.split(`{{${key}}}`).join(str);
  ...
  // ──────────────────────────────────────────────────────────────────────
  // PREPARE phase — run template.helper.js prepare(ctx, helpers), or fall
  // back to preparators.prepareDefault(ctx) if the template doesn't define
  // one. This is where the ctx.placeholders dictionary gets populated with
  // technology-specific identifiers (APP_MODULE, APP_ID_SNAKE, etc.) —
  // templates decide which ones they need.
  // ──────────────────────────────────────────────────────────────────────
  let templateHelper = null;
  if (effectiveTemplatePath) {
    const helperPath = path.join(path.resolve(effectiveTemplatePath), 'template.helper.js');
    if (fs.existsSync(helperPath)) {
      try {
        templateHelper = require(helperPath);
      } catch (e) {
        if (verbose) console.log(yellow(`    helper.js load failed: ${e.message}`));
      }
    }
  }
  ...
  if (templateHelper && typeof templateHelper.prepare === 'function') {
    ...
    preparators.prepareDefault(ctx);
    if (verbose) console.log(dim(`    prepareDefault() — template has no helper.prepare() hook`));
  }
  ...
  // 4. Set theme-dependent template variables on ctx.placeholders
  ctx.themesData = themesData;
  preparators.setThemeOptions(ctx);
  preparators.setThemesConfig(ctx);
  ...
  // Build SIDEBAR_ITEMS placeholder from pages
  preparators.setSidebarItems(ctx);
  ...
  const pageSteps = pages.map(p => {
    ...
    return {
      action: 'create',
      path: route,
      // pageType.file is the canonical field per schema; pt.template is the legacy alias.
      template: pt.file || pt.template || `pages/${p.type}.svelte`,
  ...
  // Start from the resolved features (from recipe.features), then apply
  // hardcoded fallbacks for templates that don't declare features yet.
  const featureFlags = { ...(resolvedFeatures || {}) };
  // Backward-compat defaults: if a feature isn't declared in the recipe,
  // fall back to the legacy hardcoded values so existing templates keep working.
  if (featureFlags['navbar'] === undefined) featureFlags['navbar'] = true;
  if (featureFlags['sidebar'] === undefined) featureFlags['sidebar'] = true;
  if (featureFlags['footer'] === undefined) featureFlags['footer'] = true;
  if (featureFlags['profile-panel'] === undefined) featureFlags['profile-panel'] = includeProfilePanel;
  if (featureFlags['settings-panel'] === undefined) featureFlags['settings-panel'] = includeSettingsPanel;
  if (featureFlags['floating-ui'] === undefined) featureFlags['floating-ui'] = true;
  if (featureFlags['page-loader'] === undefined) featureFlags['page-loader'] = true;
  // Icon group: special case — needs to know which provider was picked.
  // Always recompute (even if recipe declared 'icons') to keep provider logic in sync.
