...
async function cmdCreate(appName, opts, usage) {
  ...
  // Run the built-in preparators that compute standard placeholders from
  // raw ctx inputs. Order-independent. This populates APP_ID, APP_NAME,
  // APP_ID_SNAKE, APP_MODULE, COPYRIGHT, DEFAULT_THEME, PM, etc.
  //
  // Late-bound preparators (SCAFFOLD_FLAGS, THEME_OPTIONS, SIDEBAR_ITEMS)
  // run later in the pipeline once their source data is populated on ctx.
  preparators.prepareDefault(ctx);

  // CREATE_COMMAND is available now (no theme/feature deps), populate it.
