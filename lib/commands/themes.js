...
async function cmdThemesRouter(subArgs, opts, usage) {
  ...
  if (!sub || sub === 'help') {
    console.log(`
  ${bold('themes')} — browse and manage Pure Admin themes

    list                 List all themes from API
    list --local         List themes configured in this project
    show <id>            Show theme details, versions, core compat
    add [id...]          Add themes to project
    search <query>       Search themes
    versions <id>        Show available versions
    compatible <ver>     List themes compatible with a core version
    download <id>        Download a theme ZIP
    init <id> [name]     Scaffold a new theme project
    update               Re-download changed themes
    build [id...]        Compile SCSS to CSS
    pack [id...]         Package into ZIP
    publish [id...]      Pack + upload to pureadmin.io
    validate [id...]     Check CSS quality
`);
    return;
  }
