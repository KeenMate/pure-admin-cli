...
const { bold, dim, cyan, green } = require('../helpers/formatting');
...
function saveUserConfig(cfg) {
  try {
    writeFile(getUserConfigPath(), JSON.stringify(cfg, null, 2) + '\n');
  } catch {}
}
...
function describePreset(preset, templates) {
  const parts = [];
  const tpl = templates.find(t => t.id === preset.templateId);
  if (tpl) parts.push(tpl.name);
  else if (preset.templateId) parts.push(preset.templateId);
  ...
  const enabled = Object.entries(features)
    ...
    .map(([, v]) => {
      if (v.flag === '--font-awesome') return 'FA';
      if (v.flag === '--lucide') return 'Lucide';
      if (v.flag === '--profile-panel') return 'profile';
      if (v.flag === '--settings-panel') return 'settings';
      return v.flag.replace('--', '');
    });
  if (enabled.length > 0) parts.push(enabled.join(', '));
...
async function cmdCreateInteractive(opts) {
  ...
  if (presetNames.length > 0 || last.templateId) {
    ...
    if (last.templateId) {
      presetOptions.push({
        value: '__last__',
        label: 'Last used',
        hint: describePreset(last, templates),
      });
    }
    for (const name of presetNames) {
      presetOptions.push({
        value: name,
        label: name,
        hint: describePreset(savedPresets[name], templates),
      });
    }
  ...
  // Per theme we also append an "Auto" option (mode=auto, follows OS
  // prefers-color-scheme at runtime) so users aren't forced into one of
  // the theme's declared modes.
  const themeOptions = [];
  for (const themeId of selectedThemes) {
    ...
    if (variants.length === 0) {
      themeOptions.push({ value: `${themeId}||auto`, label: `${themeId} ${dim('(Auto — follow OS)')}` });
      ...
      for (const v of variants) {
        ...
        if (modes.length <= 1) {
          ...
          // Synthetic Auto option — always offered when the theme has >1 mode
          const autoLabel = variants.length === 1
            ? `${themeId} ${dim('(Auto — follow OS)')}`
            : `${themeId} / ${vName} ${dim('(Auto — follow OS)')}`;
          themeOptions.push({ value: `${themeId}|${vClass}|auto`, label: autoLabel });
        }
      }
    }
  }
  ...
  if (themeOptions.length > 1) {
    const lastCombo = `${defaults.defaultTheme || ''}|${defaults.defaultVariant || ''}|${defaults.defaultMode || 'auto'}`;
    const picked = await select({
