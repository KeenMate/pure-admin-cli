...
async function cmdCreateInteractive(opts) {
  ...
  // 8b. Fetch default theme metadata → pick variant → pick mode
  let defaultVariant = '';
  ...
  try {
    ...
    themeVariants = (themeDetail?.theme?.variants || []).map(v => ({
      id: v.cssClass || '',
      name: v.name || 'Default',
      modes: (v.modes || []).map(m => ({
        id: m.mode,
    ...
    s.stop(`${themeVariants.length} variant(s)`);
  } catch {
    s.stop('Could not load theme metadata');
  }

  // Pick variant (skip if only one)
  let selectedVariant;
  if (themeVariants.length > 1) {
    const variantIds = themeVariants.map(v => v.id);
    const picked = await select({
      message: 'Default color variant',
      initialValue: ifAvailable(last.defaultVariant, variantIds) || '',
      options: themeVariants.map(v => ({
        value: v.id,
        label: v.name,
        hint: v.id === '' ? 'default' : v.id
      }))
    });
    if (isCancel(picked)) { cancel('Cancelled.'); process.exit(0); }
    selectedVariant = themeVariants.find(v => v.id === picked);
    defaultVariant = picked;
  } else if (themeVariants.length === 1) {
    selectedVariant = themeVariants[0];
    defaultVariant = selectedVariant.id;
  }

  // Pick mode from selected variant (skip if only one)
  const modes = selectedVariant?.modes || [];
  if (modes.length > 1) {
    const modeDefault = modes.find(m => m.isDefault);
    defaultMode = await select({
      message: 'Default color mode',
      initialValue: ifAvailable(last.defaultMode, modes.map(m => m.id)) || modeDefault?.id,
      options: modes.map(m => ({
        value: m.id,
        label: m.name,
