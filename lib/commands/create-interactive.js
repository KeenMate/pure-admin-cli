...
async function cmdCreateInteractive(opts) {
  ...
  const technology = await select({
    message: 'Technology',
    initialValue: last.technology || undefined,
  ...
  const templateId = await select({
    message: 'Template',
    initialValue: last.templateId || undefined,
      ...
      if (enableIt) {
        ...
        if (feature.cli.length === 1) {
          ...
          const picked = await select({
            ...
            initialValue: lastFlag || defaultOpt?.flag,
  ...
  const lastThemes = last.themes || ['corporate', 'audi', 'dark'];
  ...
  const defaultTheme = selectedThemes.length === 1
    ...
    : await select({
        ...
        initialValue: lastDefaultTheme && selectedThemes.includes(lastDefaultTheme) ? lastDefaultTheme : undefined,
  ...
  if (companies.length > 0) {
    ...
    if (useCompany) {
      company = await select({
        message: 'Company',
        initialValue: last.company || undefined,
