...
// ---------------------------------------------------------------------------
const fs = require('fs');
const { mkdir } = require('../helpers/files');
const path = require('path');
const { bold, dim, green, yellow, red } = require('../helpers/formatting');
...
async function cmdPack(themeNames, opts) {
  ...
  mkdir(path.resolve(root, outputDir));
  ...
  for (const t of targets) {
    ...
    let cssContent = rewriteUrls(rawCss);
    let scssContent = scssPath ? rewriteUrls(fs.readFileSync(scssPath, 'utf-8')) : null;

    // Audit assets folder vs theme.json manifest vs CSS url() refs
    const audit = auditAssetManifest({ themeDir, theme, cssContent: rawCss });
    if (audit.undeclaredRefs.length > 0) {
      console.error(`    ${red('Error:')} CSS references files not declared in theme.json (will 404 after publish):`);
      for (const r of audit.undeclaredRefs) console.error(`      - ${r}`);
      console.error(`    ${dim('Add these to theme.json under fonts.files or assets.files and re-pack.')}`);
      process.exitCode = 1;
      continue;
    }
    if (audit.missingDeclared.length > 0) {
      console.error(`    ${red('Error:')} theme.json declares files that are missing on disk:`);
      for (const r of audit.missingDeclared) console.error(`      - ${r}`);
      process.exitCode = 1;
      continue;
    }
    if (audit.orphans.length > 0) {
      console.log(yellow(`    Warning: files in assets/ not declared in theme.json (won't be packed):`));
      for (const r of audit.orphans) console.log(yellow(`      - ${r}`));
    }
    ...
    // Source-tree paths (dist/<id>.css, src/scss/<id>.scss) are rewritten to the
    // published-ZIP layout (css/<id>.css, scss/<id>.scss) so consumers don't have
    // to know about the build-output convention of the theme repo.
    const packedCss = `css/${id}.css`;
    const packedScss = scssContent ? `scss/${id}.scss` : undefined;
    const rewrittenVariants = Array.isArray(theme.colorVariants)
      ? theme.colorVariants.map(v => ({ ...v, file: packedCss }))
      : theme.colorVariants;
    const rewrittenExports = {
      ...(theme.exports || {}),
      css: packedCss,
      ...(packedScss ? { scss: packedScss } : {}),
    };
    if (!packedScss) delete rewrittenExports.scss;

    const enriched = {
      ...theme,
      colorVariants: rewrittenVariants,
