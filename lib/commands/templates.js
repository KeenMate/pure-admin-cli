  ...
  for (const { dir: templateDir, name } of targets) {
    ...
    try {
      ...
      if (isWin) {
        ...
        // Windows Compress-Archive adds explicit directory entries to the ZIP
        // for intermediate directories. The pureadmin.io integrity checker
        // rejects those. Strip them using Python (available on Windows).
        try {
          ...
          const pyScript = [
            ...
            `s='${absZip}'`,
            "t=s+'.tmp'",
            "zi=zipfile.ZipFile(s,'r')",
            "zo=zipfile.ZipFile(t,'w',zipfile.ZIP_DEFLATED)",
            '[zo.writestr(i,zi.read(i.filename)) for i in zi.infolist() if not i.is_dir()]',
            'zi.close();zo.close()',
            'os.replace(t,s)',
          ].join(';');
          execSync(`python3 -c "${pyScript}"`, { stdio: 'pipe' });
        } catch {
          // Python not available — leave the ZIP as-is. It'll work on
          // platforms whose unzip ignores dir entries (most do).
        }
      } else {
