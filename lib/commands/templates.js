...
// Walk a directory tree, returning a flat list of files with their paths
// relative to the walk root. Returning files only — never directory entries —
// matches the server's integrity model, which declares shas per-file and
// rejects the bare directory entries that archiver's .directory() helper
// would otherwise add to the zip.
...
function walkFileHashes(dir) {
  const out = {};
  for (const f of walkFiles(dir)) out[f.rel] = sha256File(f.src);
  return out;
}
  ...
  for (const { dir: templateDir, name } of targets) {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        // Walk and add files individually instead of archive.directory(), which
        // emits explicit dir entries (template/lib/, template/src/, etc.) that
        // the server's integrity check rejects as "file in ZIP but not declared".
        for (const f of walkFiles(path.join(templateDir, 'template'))) {
          archive.file(f.src, { name: `template/${f.rel}` });
        }
        for (const f of walkFiles(path.join(templateDir, 'pages'))) {
          archive.file(f.src, { name: `pages/${f.rel}` });
        }
  ...
  for (const { id, version } of targets) {
    ...
    try {
      const { status, body } = curlUpload({
        url: uploadUrl, apiKey, fieldName: 'template', filePath: zipPath,
      });
      if (status === 200) {
        let result;
        ...
        // Server says this CLI is too old for write ops. Surface the message
        // it sent — that's the whole point of the 426 contract.
      ...
      // serverTooOld throws bubble up to main(); other errors land here.
