...
async function cmdTemplates(subArgs, opts) {
  ...
  if (sub === 'list') {
    ...
    console.error(`  Available: list, pack, publish, validate\n`);
    process.exit(1);
  }
}
...
async function cmdTemplateValidate(templateNames, opts) {
  ...
  if (targets.length > 1) {
    ...
    console.log(`  ${bold('Summary:')} Validated ${targets.length} template(s): ${green(`${passedTemplates} passed`)}${failed > 0 ? `, ${require('../formatting').red(`${failed} with errors`)}` : ''}\n`);
  }
