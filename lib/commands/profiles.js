...
async function cmdProfiles(subArgs) {
  ...
  if (!sub || sub === 'help') {
    ...
    const helpTarget = subArgs[1];
    if (helpTarget && cmdDefs.profiles.subcommands?.[helpTarget]) {
      console.log(formatSubcommandHelp('profiles', helpTarget, cmdDefs.profiles.subcommands[helpTarget]));
    } else {
      console.log(formatCommandHelp('profiles', cmdDefs.profiles));
    }
    return;
  }
...
async function cmdPresets(subArgs) {
  ...
  if (!sub || sub === 'help') {
    ...
    console.log(formatCommandHelp('presets', cmdDefs.presets));
    return;
  }
