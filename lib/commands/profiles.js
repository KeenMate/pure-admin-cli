...
async function cmdProfiles(subArgs) {
  ...
  if (!sub) {
    console.error(`\n  Use ${bold('profiles list')}, ${bold('profiles show <id>')}, or ${bold('profiles rm <id>')}.\n`);
    return;
  }
  ...
  if (sub === 'rm' || sub === 'remove') {
    const id = subArgs[1];
    if (!id) { console.error(`\n  ${bold('Error:')} usage: pureadmin profiles rm <id>\n`); return; }
  ...
  if (sub !== 'list') {
    ...
    console.error(`  Available: list, show, rm\n`);
    return;
  }
...
async function cmdPresets(subArgs) {
  ...
  if (!sub) {
    console.error(`\n  Use ${bold('presets list')}, ${bold('presets show <name>')}, or ${bold('presets rm <name>')}.\n`);
    return;
  }

  if (sub === 'rm' || sub === 'remove') {
    const name = subArgs[1];
    if (!name) { console.error(`\n  ${bold('Error:')} usage: pureadmin presets rm <name>\n`); return; }
