...
//
// Sends X-Pureadmin-Cli-Version on every request and records the server's
// advertise headers (server version, cli-latest, cli-min-write, cli-max-compat)
// for the version-check post-command hook. recordResponse may throw if the
// server is too old for this CLI — that bubbles up as a hard fail.
//
...
function httpGet(urlPath, { timeout, allow304 = false } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const headers = { 'X-Pureadmin-Cli-Version': versionCheck.getCliVersion() };
    httpClient().get(url, { timeout: t, headers }, (res) => {
      try { versionCheck.recordResponse(res.headers); }
