...
function formatLlmOutput() {
  const output = {
    tool: 'pureadmin',
    description: 'CLI for Pure Admin — scaffold apps, manage themes and templates, publish to pureadmin.io',
    globalFlags: globalFlags.map(f => ({ flag: f.flag, value: f.value, description: f.description })),
    commands: {},
  };

  for (const [name, cmd] of Object.entries(commands)) {
    const entry = { description: cmd.description };

    if (cmd.args) {
      entry.args = cmd.args.map(a => ({
        name: a.name,
        required: !!a.required,
        variadic: !!a.variadic,
    ...
    if (cmd.subcommands) {
      entry.subcommands = {};
      for (const [subName, sub] of Object.entries(cmd.subcommands)) {
        const subEntry = { description: sub.description };
        if (sub.args) {
          subEntry.args = sub.args.map(a => ({
            name: a.name,
            required: !!a.required,
            variadic: !!a.variadic,
            description: a.description,
        ...
        if (sub.flags) {
          subEntry.flags = sub.flags.map(f => ({
            flag: f.flag,
            value: f.value || null,
            description: f.description,
          }));
