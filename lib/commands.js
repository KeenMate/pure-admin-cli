...
const commands = {
  ...
  themes: {
    ...
    subcommands: {
      ...
      update: {
        description: 'Bump declared themes to latest versions compatible with pure-admin-core (rewrites lockfile)',
      },
      install: {
        description: 'Install themes from declarations + lockfile; resolves any not yet locked (default dev/setup verb)',
      },
      ci: {
        description: 'Install exactly what the lockfile records, fail if out of sync (npm ci equivalent)',
      },
      build: {
...
const globalFlags = [
  { flag: '--server', value: '<target|url>', description: 'Target name from config.targets, or raw URL (must have scheme)' },
  { flag: '--api-key', value: '<key>', description: 'Override resolved API key (for publish commands)' },
];
...
// Render an arg as it appears in a command signature: `<name>`, `<name...>`,
// `[name]`, `[name...]`. Variadic applies to required AND optional args.
function formatArg(a) {
  const dots = a.variadic ? '...' : '';
  return a.required ? `<${a.name}${dots}>` : `[${a.name}${dots}]`;
}
...
function formatCommandHelp(name, cmd, prefix = 'pureadmin') {
  ...
  if (cmd.subcommands) {
    ...
    for (const [subName, sub] of subs) {
      const argsStr = (sub.args || []).map(formatArg).join(' ');
  ...
  if (cmd.args) {
    for (const arg of cmd.args) {
      lines.push(`    ${formatArg(arg)}  ${dim(arg.description)}`);
    }
  }
  ...
  for (const f of globalFlags) {
    lines.push(`    ${(`${f.flag}${f.value ? ' ' + f.value : ''}`).padEnd(24)} ${f.description}`);
  }
  lines.push('');
...
function formatGlobalHelp() {
  ...
  for (const [name, cmd] of Object.entries(commands)) {
    if (!cmd.subcommands) {
      const argsStr = (cmd.args || []).map(formatArg).join(' ');
...
function formatSubcommandHelp(parentName, subName, sub) {
  ...
  if (sub.args?.length) {
    ...
    for (const a of sub.args) {
      lines.push(`    ${formatArg(a).padEnd(22)} ${a.description}`);
    }
    lines.push('');
  }
  ...
  if (sub.flags?.length) {
    ...
    for (const f of sub.flags) {
      ...
      lines.push(`    ${left.padEnd(22)} ${f.description}`);
    }
    lines.push('');
  }
  ...
  lines.push(`  ${bold('Global:')}`);
  for (const f of globalFlags) {
    lines.push(`    ${(`${f.flag}${f.value ? ' ' + f.value : ''}`).padEnd(22)} ${f.description}`);
  }
  lines.push('');
...
Themes:
  ...
  Online themes are gitignored and re-fetched on CI via "themes ci" (which honors pureadmin.lock.json for reproducibility).
  ...
  Three install verbs (mirror npm):
    "themes install" — like "npm install": declarations are the source of truth. Lock present → install from lock.
                       Lock missing or some declared theme not yet locked → resolve fresh from API (filtered by the
                       project's pure-admin-core version), download, write lockfile entry. Default dev/setup verb.
    "themes update"  — like "npm update": bump every declared theme to the latest version compatible with the
                       project's pure-admin-core, re-download changed ones, rewrite the lockfile. Review the diff in a PR.
    "themes ci"      — like "npm ci": lockfile is the source of truth. Fail fast if declarations and lock are out of
                       sync. Writes nothing. Use this in CI pipelines.

  All three pass the project's @keenmate/pure-admin-core version (auto-detected from package.json, falling back to
  assets/package.json for Phoenix) to the API as ?core_version=X so themes resolve to compatible versions.
...
Config sources (highest priority first):
  --api-key / --server flags         — CLI flags win over everything.
  ...
  ./pureadmin.lock.json (project)    — committed. Tool-managed resolutions (version + content_sha + fetched_at + source per theme).
                                       Read by "themes install" and "themes ci"; written by "themes install", "themes update", and "themes add".
  ~/.pureadmin.json (user)           — companies, workspaces, presets, default API key, default target.

Project JSON merge order (later overrides earlier): ~/.pureadmin.json → ./pureadmin.json → ./.pureadmin.json. The deep-merged result is what "top-level config" refers to below.

API key resolution chain (used by "themes publish" / "templates publish"):
  1. --api-key flag
  2. resolved target's apiKey (from "targets" block in the merged config; selected by --server / defaultTarget)
  3. PUREADMIN_API_KEY env var
  4. top-level "apiKey" field in the merged config (fallback)
...
pureadmin themes
  ...
  themes add <id...> — download and add theme(s) to this project. Extracts ZIP to the themes directory,
    ...
    --dir <path> (alias --themes-dir) — theme output directory where ZIPs are extracted (default: static/themes
      for SvelteKit, public/themes for Vite. For Phoenix use priv/static/themes, for .NET use wwwroot/themes)
  ...
  themes install — default dev/setup verb. Like "npm install": declarations in pureadmin.json (+ .pureadmin.json
    overrides) are the source of truth for which themes to install. Each declared theme: if locked, downloads that
    exact version; if unlocked, resolves fresh against the API (filtered by the project's pure-admin-core version) and
    writes a new lockfile entry. Local --path themes are re-copied from disk every time. Stale lock entries are pruned.
  themes update — bump versions. Like "npm update": for every declared theme, queries the API (filtered by core
    version), re-downloads if content_sha differs from the lockfile, and rewrites pureadmin.lock.json. Review the
    diff in your PR. Never modifies pureadmin.json or .pureadmin.json.
  themes ci — strict reproduce. Like "npm ci": fails fast if any declared theme is missing from the lockfile (run
    "themes install" first to resolve). Otherwise downloads each theme at its locked version. Writes nothing. Use
    in CI pipelines.
  ...
  themes publish [id...] — pack + upload theme(s) to pureadmin.io.
    Requires an API key — see "API key resolution chain" above.
    --no-build — use existing CSS without recompiling
...
Global flags:
  --server <target|url> — target name (looked up in config.targets) or raw URL with scheme (http(s)://...).
    Unknown names error out with the list of available targets instead of being silently treated as URLs.
  --api-key <key> — override resolved API key (for publish commands)
