...
const { cmdThemesRouter } = require('./commands/themes');
const { cmdUpdate } = require('./commands/themes');
...
function usage(error) {
  ...
  ${bold('Config commands:')}
    profiles                    List company profiles and workspace mappings
    profiles rm <id>            Remove a company profile
    presets                     List saved create presets
...
async function main() {
  ...
  for (let i = 0; i < rest.length; i++) {
    if (rest[i] === '--server' && rest[i + 1]) {
      ...
      console.error(`  Known flags: --server, --api-key, --dir, --themes-dir, --name, --company, --preset, --template, --template-path, --font-awesome, --lucide, --fluent-ui, --settings-panel, --profile-panel, --no-makefile, --no-install, --offline, --no-build, --verbose, --version, --output\n`);
      process.exit(1);
    } else {
