...
// Levenshtein edit distance (small inputs — command names).
...
// Suggest a top-level command for a typo. Singular/plural first (the most
// common case — `theme` vs `themes`), then edit distance ≤ 1. Returns null
// when no plausible match exists, so callers can fall back to the plain
// "Unknown command" error.
function suggestCommand(name) {
  if (!name) return null;
  const known = Object.keys(cmdDefs);
  if (known.includes(name + 's')) return name + 's';
  if (name.endsWith('s') && known.includes(name.slice(0, -1))) return name.slice(0, -1);
  for (const cmd of known) {
    if (editDistance(name, cmd) <= 1) return cmd;
  }
  return null;
}

function unknownCommandError(name) {
...
async function main() {
  ...
  // Intercept --help / -h anywhere after the command name. Without this, the
  // catch-all flag parser below silently sets opts.help and the command runs —
  // which for destructive verbs (themes publish, templates publish) means a
  // real upload instead of a help screen.
  if (rest.includes('--help') || rest.includes('-h')) {
    const def = cmdDefs[command];
    if (!def) return usage(unknownCommandError(command));
    // Skip flag values when looking for the subcommand positional.
    const valueFlags = new Set([
      '--server', '--version', '--output', '--template', '--template-path',
      '--path', '--name', '--themes', '--theme', '--company', '--preset',
      '--api-key', '--dir', '--themes-dir',
    ]);
    let sub;
    ...
    if (def.subcommands && sub && def.subcommands[sub]) {
      const { formatSubcommandHelp } = require('./commands');
      console.log(formatSubcommandHelp(command, sub, def.subcommands[sub]));
    } else {
      console.log(formatCommandHelp(command, def));
    }
    return;
  }
  ...
  try {
    switch (command) {
      ...
      case 'help': {
        ...
        // Not a top-level command — search nested subcommands (e.g. `help publish`
        // matches both `themes publish` and `templates publish`).
        const matches = [];
        for (const [parent, p] of Object.entries(cmdDefs)) {
          if (p.subcommands && p.subcommands[target]) {
            matches.push({ parent, sub: p.subcommands[target] });
          }
        }
        if (matches.length === 1) {
          console.log(formatSubcommandHelp(matches[0].parent, target, matches[0].sub));
          return;
        }
        if (matches.length > 1) {
          const hint = matches.map(m => `pureadmin help ${m.parent} ${target}`).join(', ');
          return usage(`'${target}' is a subcommand of multiple commands. Try: ${hint}`);
        }
        return usage(unknownCommandError(target));
      }
      default: return usage(unknownCommandError(command));
    }
  } catch (err) {
    if (err.serverTooOld) {
