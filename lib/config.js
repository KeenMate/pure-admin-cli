...
/**
 * Deep merge source into target. Objects are merged recursively; other values
 * are overwritten. Arrays are replaced (not concatenated).
 */
...
function loadConfig() {
  ...
  while (true) {
    ...
    if (base || local) {
      if (base) {
        deepMergeConfig(config, base);
        config._configPath = path.join(dir, 'pureadmin.json');
      }
      if (local) {
        deepMergeConfig(config, local);
        config._configPath = path.join(dir, '.pureadmin.json');
      }
      break;
    }
