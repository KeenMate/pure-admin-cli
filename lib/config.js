...
// Read the project config as a merged view of pureadmin.json (project, checked
// in) plus .pureadmin.json (local overrides, gitignored). Walks up from cwd to
// find the first directory containing either file.
//
// Returns { path, data } where `path` is the pureadmin.json location (used by
// saveProjectConfig). Saves go ONLY to pureadmin.json — local overrides stay
// in .pureadmin.json. Caveat: if the same key exists in both files,
// .pureadmin.json wins on read, so a subsequent save of the merged value
// effectively writes the override into pureadmin.json too. That's only an
// issue when consumers blindly mutate-and-save the whole merged blob; for
// targeted writes (e.g. setting a single theme entry) it's harmless.
function loadProjectConfig() {
  ...
  while (true) {
    ...
    if (hasBase || hasLocal) {
      if (hasBase) baseFile = base;
      if (hasLocal) localFile = local;
      break;
    }
    const parent = path.dirname(dir);
    if (parent === dir) break;
  ...
  const data = {};
  if (baseFile) {
    const base = tryLoadJson(baseFile);
    if (base) deepMergeInto(data, base);
  }
  if (localFile) {
    const local = tryLoadJson(localFile);
    if (local) deepMergeInto(data, local);
  }

  return {
    path: baseFile || path.join(process.cwd(), 'pureadmin.json'),
    data,
  };
}
