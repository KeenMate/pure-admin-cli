...
function setProfiles(ctx) {
  ...
  // Icon provider — when FA is default but not actually loaded, label it
  // clearly rather than implying FA is in use.
  const iconValue = ctx.iconProvider === 'font-awesome' && !ctx.includeFontAwesome
    ? 'none'
    : ctx.iconProvider;
