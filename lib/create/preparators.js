...
/**
 * Identify the demo entities controlled by the `demo-pages` feature by
 * scanning recipe.steps for `{action: "delete", path: "...", unless: "demo-pages"}`
 * patterns — each such step declares one entity (the basename of its path).
 * E.g. `delete src/routes/users unless demo-pages` → entity "users".
 */
function findDemoEntities(recipe) {
  if (!recipe?.steps) return [];
  const entities = [];
  for (const step of recipe.steps) {
    if (step.action !== 'delete' || step.unless !== 'demo-pages') continue;
    const segments = (step.path || '').split(/[\\/]/);
    const last = segments[segments.length - 1];
    if (last) entities.push(last);
  }
  return entities;
}

/**
 * Compose the "Demo pages" row text for the README and dashboard.
 *
 * `flagLabel` is how to render the `--bare` flag — markdown-quoted (\`--bare\`)
 * for the README, bare for the dashboard.
 *
 * Returns null when the template doesn't declare demo-pages (row omitted),
 * otherwise an honest description of the on-disk state:
 *   - --bare → "none (--bare)"
 *   - demos on, no preset overlap → "Users, Settings (pass --bare to remove)"
 *   - demos on, full preset overlap → "Users, Settings — overwritten by preset pages"
 *   - demos on, partial overlap → "Settings (Users overwritten by preset; pass --bare to skip)"
 */
function describeDemoPages(ctx, flagLabel) {
  if (!ctx.features || !('demo-pages' in ctx.features)) return null;
  if (!ctx.features['demo-pages']) return `none (${flagLabel})`;

  const demoEntities = findDemoEntities(ctx.recipe);
  if (demoEntities.length === 0) return `(pass ${flagLabel} to remove)`;

  const cap = (s) => s.charAt(0).toUpperCase() + s.slice(1);
  const presetEntities = new Set(
    (ctx.pages || []).map(p => p.entity || p.type).filter(Boolean)
  );

  const shadowed = demoEntities.filter(e => presetEntities.has(e));
  const intact = demoEntities.filter(e => !presetEntities.has(e));

  if (shadowed.length === demoEntities.length) {
    return `${demoEntities.map(cap).join(', ')} — overwritten by preset pages`;
  }
  if (shadowed.length === 0) {
    return `${demoEntities.map(cap).join(', ')} (pass ${flagLabel} to remove)`;
  }
  return `${intact.map(cap).join(', ')} (${shadowed.map(cap).join(', ')} overwritten by preset; pass ${flagLabel} to skip)`;
}
...
function setProfiles(ctx) {
  ...
  // When the preset's pages shadow the demos (e.g. `--preset full` declares
  // master-detail/users + form/settings), the demo files were copied and
  // then immediately overwritten by the page generators — note that.
  const demoPagesValue = describeDemoPages(ctx, '`--bare`');
...
function collectCreateSummary(ctx) {
  ...
  // Demo pages — bundled scaffold routes (Users, Settings) gated by the
  // optional `demo-pages` template feature. Same logic as setProfiles but
  // without markdown — for the dashboard's Project Info card.
  const demoPages = describeDemoPages(ctx, '--bare');
