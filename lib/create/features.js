...
function resolveFeatures(features, opts, preset) {
  ...
  for (const [id, feat] of Object.entries(features)) {
    ...
    // `cliDisable: "--bare"` — any flag can be a disable trigger, useful when
    // the natural CLI verb isn't `--no-feature`. When the named flag is set,
    // the feature is forced OFF (overriding isDefault and any preset/company).
    if (typeof feat.cliDisable === 'string') {
      const optKey = flagToOpt(feat.cliDisable);
      if (opts[optKey]) {
        enabled[id] = false;
        continue;
      }
      // Flag not set → fall through to normal resolution (isDefault wins).
    }
